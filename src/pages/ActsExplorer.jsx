...
const ActsExplorer = ({ embedded = false }) => {
  ...
  // --- REFACTOR: Defined as a render variable, not a nested component ---
  const sidebarUI = (
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
                  ...
                  {list.map((a) => (
                    <li key={a.id} className="pl-4 pr-2 py-1">
                      <button
                        onClick={() => {
                          navigate(`/act/${a.id}`);
                          setIsMobileSidebarOpen(false); // Close mobile menu on click
                        }}
        ...
        {Object.keys(actsByYear).length === 0 && (
          <p className="text-gray-400 text-center py-4 text-sm">No acts found.</p>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F3F4F6] overflow-hidden">
        ...
        {isMobileSidebarOpen && (
          <>
            ...
            >
              ...
              {/* Render the extracted Sidebar UI */}
              {sidebarUI}
            </motion.div>
          </>
        )}
      ...
      {/* --- MOBILE HEADER --- */}
      <div className="lg:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between z-30 relative shadow-sm">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* --- DESKTOP SIDEBAR --- */}
        <aside className="hidden lg:block w-72 h-full flex-shrink-0 z-20 shadow-sm bg-white">
          {sidebarUI}
        </aside>
